//
// Created by andre on 27/01/2026.
//
...
namespace prng_tests {
    ...
    bool test_serial_correlation() {
        ...
        constexpr double expected_2d = static_cast<double>(TEST_SAMPLES) / static_cast<double>(GRID_2D * GRID_2D);
        double chi_sq_2d = 0.0;
        for (uint64_t i = 0; i < GRID_2D * GRID_2D; ++i) {
            const auto obs = static_cast<double>(grid_2d[static_cast<size_t>(i)]);
        ...
        constexpr double expected_3d = static_cast<double>(TEST_SAMPLES) / static_cast<double>(GRID_3D * GRID_3D * GRID_3D);
    ...
    bool test_birthday_spacing() {
        ...
        bool collision_pass = (collision_ratio < 10.0) || expected_collisions < 1.0;
